...
export function renderCalendar(year, month, events) {
    ...
    const thisYearAndMonthEvents = events.filter(event => event.date.getFullYear() === year && event.date.getMonth() === month);
    ...
    for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < 7; j++) {
            ...
            if (i === 0 && j < firstDay) {
                ...
                if (renderToday.getFullYear() === year &&
                ...
                if (thisYearAndMonthEvents.length) {
                    // add events
                    const todayEvents = thisYearAndMonthEvents.filter(event => event.date.getDate() === date);
